...
def recommendation(df, startpoint, visited, preference, priority, duration = 24,price):
    ...
    #calculate total money to derive the number of places to visit
    n = 0
    total_price = 0
    
    #extract the name of sorted attractions
    while total_price < money & n < len(df_4):
        time = df_3.iloc[n]['price']#extract time of the corresponding attractions
        sum_money =+ money
        n += 1
        
    #extract the top attractions by distance within time limit
    result = df_4.iloc[:n-1]
    return result
